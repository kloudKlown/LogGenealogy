...
In our preliminary analysis, we find that 20\%-45\% of logs are changed in our subject applications. These logging statement changes affect the log processing tools which run on the studied applications, forcing developers spend more time on maintenance of the log processing tools. By analyzing the metrics which can increase the likelihood of a change to logging statements, developers of log processing tools can decrease the effort spent on maintenance. 
...
\subsection{Approach}
In this section we construct a random forest classifier for explaining the likelihood of changes to logging statements in the studied applications. We then evaluate the performance of our random forest classifier and use it to understand which factors can increase the likelihood of a change to a logging statement in the studied applications.

We use context and log metrics to train the random forest classifier. Context metrics measure the file context at the time of adding the logging statements. Log metrics measure the details about the added logging statement. We use the Git repository to extract the context metrics and log metrics for the studied applications. 

Table~\ref{tba:Taxonomy} defines each metric collected and the rationale behind our choice of each metric. We use the context metrics as it describes the conditions in which the logging statement was added into the application and log metrics provides information about the added logging statement. These metrics also benefit log processing tool developers as they do not need domain knowledge about the application to understand these metrics.

We build random forest classifier~\cite{Albert2008424} to predict whether a logging statement will change in our studied applications. A random forest is a collection of largely uncorrelated decision trees in which the results of all trees are combined to form a generalized predictor. In our classifier, the context and log metrics are the explanatory variables and the dependent class variable is a boolean variable that represents whether the logging statement ever changed or not (i.e., '0' for not changed and '1' for changed).
...
%		\cline{4-4} 
		&  &  & r: Logging statements present in a file which is often changed, have higher a likelihood of being changed~\cite{Characterizinglogs}. Hence, more commits to a file, higher the likelihood of a change to a logging statement. 
...
%		\cline{4-4} 
		&  &  & r: Logging statements added during large code changes like feature addition can be very different from logging statements added during bug fixes which have lesser code changes.
%		more stable than logs added during bug fixes which have lesser code changes. 
		 \tabularnewline
		\cmidrule{2-4}
		& File ownership  & Numerical  & d: Percentage of the file written by the developer who added the logging statement. \tabularnewline
%		\cline{4-4} 
...
%		\cline{4-4} 
		&  &  & r: More experienced developers may add more stable logging statements than a new developer who has little understanding of the application. 			 		
	 \tabularnewline
...
%	 		 Prior research finds that logs are mostly used in assertion checks, logical branching and return value checking~\cite{Fu1}. 
	 		 Logging statement used in logical branching and assertion checks, i.e., if-else blocks, may be very different from the logging statements in try-catch, exception blocks. \tabularnewline
...
%		\hline
		...
		\multirow{6}{*}{Log Metrics}
		 ...
		 &  &  & 
		 r: New logging statements added to a file are more likely to be changed than logging statements previously left untouched. 
...
%	\cline{4-4}
	 &  &  &
	r: Logging statement which are added, removed and re-added into a file suggest that developers are unsure of the purpose of the log making them very unstable and prone to changes.  \tabularnewline
\cmidrule{2-4}
%	& 	
...
\begin{table*}
\caption{The investigated metrics in our clasifier}	
%	\label{tba:Taxonomy} %
	...
	\begin{tabular}{c|l|l|>{\raggedright}p{1\columnwidth}}
		...
		%		\hline 
		\hline 
		\multirow{12}{*}{Log Metrics}
		

		& Log density & Numerical & d: Ratio of the number of logging statements to the source code lines in the file.\tabularnewline
%		\cline{4-4} 
		&  &  & r: Files which are well logged (i.e., higher log density) may not need additional logging statement and are less likely to be changed.
		\tabularnewline
\cmidrule{2-4}
...
%		\cline{4-4} 
		&  &  & r: Research has shown that developers spend significant amount of time in adjusting the verbosity of logging statements~\cite{Characterizinglogs}. Hence, higher level logging statements such as \emph{warn} and \emph{error} might be more thought out than default level \emph{info} logging statements. \tabularnewline
\cmidrule{2-4}
...
Figure~\ref{fig:ModelCreationLogGeanology} provides an overview of the construction steps (C1 to C3) for building a random forest classifier and steps (A1 and A3) provides an analysis of the results. We use the statistical tool R to model our data using the \emph{RandomForest} package.\\
...
We find that \emph{total revision count} is highly correlated with \emph{code and log churn in commit}, because a file with more commits has higher chance of having a large commit with log changes, than a file with fewer commits. We exclude \emph{total revision count} and \emph{log churn in commit} and retain \emph{code churn in commit} as it is a simpler metric to compute.

Correlation analysis does not indicate redundant metrics, i.e, metrics that can be explained by other explanatory metrics. The redundant metrics can interfere with the one another and the relation between the explanatory and dependent metrics is distorted. We perform redundancy analysis to remove such metrics. We use the \textsl{redun} function that is provided in the \textsl{rms} package to perform the redundancy analysis. We find that \emph{Log addition} is the common redundant variable across all the studied applications and we remove it before building our classifier.


\begin{figure}
	\centering
	\includegraphics[height=1\columnwidth,width=0.95\columnwidth]{Spearman}
...
\begin{table}[t]
	\centering
		...
		\label{tba:confusion}
		\resizebox*{.9\columnwidth}{.15\columnwidth}{
...
After we build the random forest classifier, we evaluate the performance of our classifier using precision, recall, F-measure, AUC and Brier Score. These measures are functions of the confusion matrix as shown in Table~\ref{tba:confusion} and are explained below.

\textsl{Precision (P)} measures the correctness of our classifier in predicting which logging statement will change in the future. Precision is defined as the number of logging statements which were accurately predicted as changed over all logging statements predicted to have changed as explained in Equation~\ref{f:precision}.   \\
	 \begin{equation}
 P =  \dfrac{TP}{TP + FP } 
 \label{f:precision}
	 \end{equation}	
	 
\textsl{Recall (R)} measures the completeness of our classifier. A classifier is said to be complete if the classifier can correctly classify all the logging statements which will get changed in our dataset. Recall is defined as the number of logging statements which were accurately predicted as changed over all logging statements which actually change as explained in Equation~\ref{f:recall}.
	 \begin{equation}
	 R =  \dfrac{TP}{TP + FN } 
	 \label{f:recall}
	 \end{equation}	
...
\textsl{Area Under Curve (AUC)} is used to measure the overall ability of the classifier to classify changed and unchanged logging statements. The value of AUC ranges between 0.5 (worst) for random guessing and 1 (best) where 1 means that our classifier can correctly classify every logging statement as changed or unchanged.
...
\textsl{Brier score (BS)} is a measure of the accuracy of the predictions in our classifier~\cite{wilks2011statistical}. Brier score explains how well the classifier performs compared to random guessing as explained in Equation~\ref{f:BS}, where $P_{t}$ is the probability of log change and $O_{t}$ is the actual log being changed or not. A perfect classifier will have a Brier score of 0, a perfect misfit classifier will have a Brier score of 1 (predicts probability of log change when log is not changed) and for random guessing Brier score reaches the value of 0.25. This means lower the Brier score value, the better our random forest classifier.
...
\begin{figure*}[tb]
	...
	\centering
	\subfloat{\includegraphics[width=1.5\columnwidth]{BoxplotResults1}\label{fig:f41}}
	\subfloat{\includegraphics[width=0.5\columnwidth,height=0.565\columnwidth]{BoxplotResults2}\label{fig:f41}}
%	\hfill
%	\centering
...
As we obtain 1,000 data sets for each metric because of bootstrapping process, we use the \textbf{Scott-Knott Effest size clustering} (SK-ESD) clustering to group the metric based on their effect size~\cite{tantithamthavorn2015tse}. This is done to group metrics which are important predictors for the likelihood of logging statement change. The SK-ESD algorithm uses the hierarchical clustering approach to divide the metrics and uses the effect ratio test to judge the difference between the groups. This assures the means of metrics within a group not to be statistically significantly different. We use the \emph{SK.ESD} function in the \emph{ScottKnottESD} package of R and set the effect size threshold parameter to negligible, (i.e., $<$ 0.2) to cluster the two metrics into the same groups. 
%\begin{table}
...
\begin{table*}[t]
	\protect\protect\caption{The importance values of the metrics (top 10), divided into homogeneous
		groups by Scott-Knott clustering}
	...
	\begin{tabular}{lll|lll}
		...
		8  & Log text length  & 0.022  & 7 & Log context & 0.052 \tabularnewline
		  &  &   & 9 & Variable declared & 0.051  \tabularnewline
		\hline 
	\end{tabular}
	...
	\begin{tabular}{lll|lll}
		...
		9 & Log context & 0.061   & 9  & Log text length & 0.071 \tabularnewline
		\hline 
...
\begin{figure}[tb]
	...
	\centering
	\subfloat{\includegraphics[width=0.49\columnwidth]{QP_Amq_Dxp}\label{fig:f4}}
	\hfill
	\centering
	...
	\hfill
	\centering
	\subfloat{\includegraphics[width=0.49\columnwidth]{QP_Lif_Dxp}\label{fig:f4}}
	\hfill
	%	\caption{Distribution of the number of developers responsible for changing a log}
	\caption{Comparing the probability of a change to a logging statement against experience of the developer adding the logging statement }
	\label{fig:Dexp_BP}
...
\textbf{{{Developer experience} is an important metric to explain the likelihood of a change to a logging statement.}} From Table~\ref{tba:Scott}, we see that developer experience is one of top four metrics, to help explain the likelihood of a logging statement being changed in all the studied applications. 
Figure~\ref{fig:Dexp_BP} shows the probabilities of a logging statements being changed as developer experience increases, and it is interesting to note that in all the projects logging statements introduced by new developers have lower probability of being changed, when compared to more experienced developers. We also observe that as developers get more experience the probability of a change to a logging statement decreases in ActiveMQ, Camel and Liferay. This downward trend maybe be because in ActiveMQ, Camel and Liferay, the top three developers are responsible for adding more than 50\% of the logging statements as seen in Table~\ref{tba:topDevelopers}. We also find that upto 70\% of the logging statements added by these top developers never change. These findings, suggest that developers of log processing tools should be more cautious when using a logging statement written by developers who have some experience in the application rather than new developers or very experienced developers.
...
\begin{figure}[tb]
	...
	\centering
	\subfloat{\includegraphics[width=0.49\columnwidth]{QP_Amq_Ow}\label{fig:f4}}
	\hfill
	\centering
	...
	\hfill
	\centering
	\subfloat{\includegraphics[width=0.49\columnwidth]{QP_Lif_Ow}\label{fig:f4}}
	\hfill
	%	\caption{Distribution of the number of developers responsible for changing a log}
...
\textbf{{Ownership of the file is an important metric to explain the likelihood of a change to a logging statement.}} From Table~\ref{tba:Scott}, we see that ownership of the file is one of top four metrics after developer experience to help explain the likelihood of a change to a logging statement in all the studied applications. From Figure~\ref{fig:Numberofchanges_BP}, we observe that in all the applications that logging statements introduced by developers who own more than 75\% of the file are less likely to be changed. We also observe that developers who own less than 20\% of the file are responsible for 27\%-67\% of the changes to logging statements in the studied applications, which is seen as upward trend from 0 to 0.20 in Figure~\ref{fig:Numberofchanges_BP}. These results suggest that developers of log processing tools should be more cautious when using a logging statement written by a developer who has contributed to less than 20\% of the file.
...
%
\textbf{{{Log density} is an important metric to explain the likelihood of a change to a logging statement.}} From Table~\ref{tba:Scott}, we see observe that log density has the highest importance in Liferay and Cloudstack. We find that in these two applications, the changed logging statements are present in files with lower log density, than unchanged logging statements. When we measure the median file sizes, we find that logging statements which change more are present in files with significantly higher SLOC (2x-3x higher). This suggests that large files which are not well logged are more likely to have unstable logging statements, than well logged files.
...
\hypobox {Our Random Forest classifier achieves a precision of 89\%-91\% and recall of 71\%-83\% across all studied applications. We find file ownership, SLOC, developer experience and log density, help in explaining the likelihood of a change to a logging statement}
