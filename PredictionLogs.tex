...
In RQ1, we find that 45\% of logs are changed at-least once in our studied systems. This affects the log processing tools which run on these studied systems, making developers spend more time on maintenance of those tools. Hence, there is a need to identify these non-stable logs to simplify the job of developers. It can also benefit log processing tool developers to develop more robust applications making them more stable.
...
Figure~\ref{fig:ModelCreationLogGeanology} provides an overview of the four steps in our model construction approach. We adopt the statistical tool R to model our data and use the `RandomForest' package to generate the random forests.\\
...
\begin{figure}
	...
	\caption{Hierarchical clustering of variables according to Spearman’s $\rho$ in ActiveMQ}
...
\begin{figure}
	\centering
	\includegraphics[height=0.4\textwidth,width=0.75\linewidth]{BootRandomForest}
...
\begin{table}[tb]
	\begin{tabular}{|>{\raggedright}p{0.45\columnwidth}|ll}
		\hline 
		\multirow{2}{0.13\columnwidth}{Actual} & \multicolumn{2}{c}{Predicted}\tabularnewline
		& Log has changed & Log has not-changed\tabularnewline
		\cline{2-3} 		\hline 
		Log has changed  & TP (True Positive)  & FN (False Negative ) \tabularnewline
